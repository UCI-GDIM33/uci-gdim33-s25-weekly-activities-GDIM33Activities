...
2. Since vertex color is stored as data in each vertex of the mesh, why is the color on our shiba from step (3) blended at the edges of different regions of color? I would assume the vertezes at the edges apear more nlended cauts of the certex data becomes more destorted.
3. Why is the shiba from step (3), which is colored with vertex color, less detailed than the shiba we rendered with a texture in last week’s activity? I belive cetex colors, are generated by computer data while the texsture render applies otself to the merticularly planned texsture. 
4. Given that vertex color generally results in a less detailed color application than applying a texture, what can you imagine vertex color is useful for? I think great for if you dont have time to texsture all objects in a scene. 
6. Based on the color of the shiba in step (4), does anything look wrong with the mesh’s vertex normals? its all different colors 
7. We used the color output of a shader to visualize a mesh’s vertex normal values in step (4). Name one other piece of vertex data (or any kind of data) you can imagine testing with a debug shader like this, and describe why that might be useful. can see if the texsturing is streached, or wrapped properly looking at uneaven color
8. Why is there an error in the lighting in step (5) on the back of the Shiba? I assuming thats cause of a dent in the back that is asumetric from the other side 
9. Why do you think we set the Blend Mode to Additive for the fire effect in Step (6)? I think blend mode additive keeps the patterns on the fire effect while still bledning in the texsture with the color
10. 
